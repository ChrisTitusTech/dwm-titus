...
ShellRoot {
    ...
    function event(domain, value, next) {
        if (domain === "time" && value === "emit") root.expectedEvents = root.timeEvents + 100;
    ...
    function controlled() {
        ...
        else if (next === "released-ready") root.stage = 3;
    ...
    function advance() {
        ...
        if (root.stage === 0 && idle && model.snapshotState === "ready") {
            ...
            root.stage = -1;
            root.mode("locale", "hold", "open");
        } else if (root.stage === 1 && model.timeDiscovery.ready && model.accountDiscovery.ready
                && model.printerDiscovery.ready && model.discovery.ready && ++root.ticks >= 8) {
            root.check(!model.localeDiscovery.ready && model.localeDiscovery.monitorOwned, "Locale readiness is held");
            ...
            root.stage = -1;
            root.event("locale", "ready", "released-ready");
        ...
        } else if (root.stage === 4 && model.busy && model.timeDiscovery.phase === "initial-active") {
            root.check(model.discoveryModels().every(item => item.phase === "initial-active"), "Refresh batches all five initial tokens");
            root.stage = -1;
            root.event("time", "emit", "first-events");
        } else if (root.stage === 5 && root.timeEvents >= root.expectedEvents) {
            root.check(model.timeDiscovery.cycle.dirty, "Time burst dirties only its active cycle");
            root.stage = -1;
            root.event("snapshot", "finish", "first-finished");
        } else if (root.stage === 6 && model.busy && model.timeDiscovery.phase === "settling-active") {
            root.check(model.accountDiscovery.phase === "idle" && model.localeDiscovery.phase === "idle", "Quiet domains need no settling token");
            root.stage = -1;
            root.event("time", "emit", "settling-events");
        } else if (root.stage === 7 && root.timeEvents >= root.expectedEvents) {
            root.check(model.timeDiscovery.unresolved, "Second burst retains unresolved time state");
            root.stage = -1;
            root.event("snapshot", "finish", "settling-finished");
        } else if (root.stage === 8 && idle && ++root.ticks >= 8) {
            root.check(root.count() === 5 && model.timeDiscovery.phase === "blocked", "Time cycle stops after initial plus settling read");
            root.check(model.nativeStateView("timezone").status === "partial"
                && model.nativeStateView("locale").status === "available", "Time and locale freshness remain independent");
            root.check(model.nativeProviderView("regional").status === "partial"
                && model.nativeStateView("timezone").value === "America/Chicago", "Unresolved provider preserves readable values");
            root.check(model.nativeStateView("timezone").detail.indexOf("Reload status") >= 0, "Unresolved state explains explicit retry");
            root.stage = -1;
            root.mode("snapshot", "quiet", "quiet");
        } else if (root.stage === 9 && idle && root.count() === 6) {
            root.check(model.accountDiscovery.fresh && model.timeDiscovery.unresolved && !model.timeDiscovery.fresh,
                "Account-triggered full read cannot clear blocked time freshness");
            root.stage = 10;
            model.operation.requestSnapshot();
        } else if (root.stage === 10 && idle && root.count() === 7) {
            root.check(model.timeDiscovery.phase === "blocked", "Required recovery read cannot clear blocked time freshness");
            root.stage = -1;
            root.command("fixture-count", ["7"], "blocked-count");
        } else if (root.stage === 11 && idle && model.timeDiscovery.phase === "blocked") {
            ...
            root.check(model.nativeProviderView("regional").status === "available", "Quiet retry restores clean provider projection");
            model.closeSettings();
            root.stage = 13;
        } else if (root.stage === 13 && root.allStopped()) {
            root.stage = -1;
            root.mode("locale", "fail", "bad-locale");
        } else if (root.stage === 14 && idle && model.localeDiscovery.failed && model.timeDiscovery.fresh) {
            root.check(root.count() === 12 && !model.localeDiscovery.fresh, "Failed monitor gets one finite shared fallback");
            root.check(model.nativeStateView("locale").status === "partial"
                && model.nativeStateView("timezone").status === "available", "Failed locale monitor does not degrade time state");
            root.stage = -1;
            root.mode("locale", "quiet", "restore-locale");
        ...
        } else if (root.stage === 16 && model.busy && model.timeDiscovery.phase === "initial-active") {
            model.closeSettings();
            root.stage = 17;
    ...
    SystemManagementModel {
        ...
        onSnapshotStateChanged: {
            ...
            if (root.stage === 11 && snapshotState === "ready") {
                ...
                root.boundaryEvents++;
                timeDiscovery.invalidate();
            }
        }
    }
    Connections {
        target: model.timeDiscovery
        function onInvalidated() { root.timeEvents++; }
        function onPhaseChanged() {
            if (root.closeDuringTake && model.timeDiscovery.phase === "initial-active") {
    ...
    Timer {
        ...
        onTriggered: root.check(false, "Timeout at stage " + root.stage + ", count " + root.count()
            + ", time " + model.timeDiscovery.phase + ", snapshot " + model.snapshotState)
    }
}
