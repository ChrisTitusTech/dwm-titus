...
| Appearance and accessibility | Versioned appearance, personalization, panel, managed-shell accessibility policy, and `settings-protocol 1` capability records; bounded asset, D-Bus owner, and XInput discovery | User theme, wallpaper, managed-shell typography, desktop personalization, contrast and motion policy, and shared panel-widget state; transactional toolkit config; external GTK and Qt tools | Read-only, user-session, delegated, unsupported | Invalid or unavailable providers are attributed per capability; notification policy and dedicated accessibility input behavior remain unsupported | `make check-appearance check-settings check-quickshell-panel-settings check-accessibility check-quickshell-controlcenter check-quickshell-qml`; nested-X11 appearance, accessibility persistence, and live toolkit reload |
...
| Notification and accessibility capability discovery | Five `settings-protocol 1` records derived from bounded personalization, managed accessibility policy, notification-owner, and XInput probes | Read-only and user-session | Text scaling reflects the complete versioned personalization record. High contrast and reduced motion expose persistent, keyboard-focusable managed-shell mutations. Notification and input readiness degrade independently and remain future Phase 5 work. |
