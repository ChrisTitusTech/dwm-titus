...
| System and diagnostics | `dwm-system-health` structured snapshots plus the Phase 6 `dwm-system-management` provider contract | Allowlisted health repairs, PackageKit updates, systemd regional actions, and trusted Fedora entry points | Read-only, user-session, privileged, delegated, unsupported | Authorization denial preserves readable state; PackageKit owns update cancellation; high-risk administration stays delegated or unsupported | Current: `make check-system-health check-quickshell-health-xvfb`; pending Phase 6: `make check-system-management check-quickshell-system-management check-quickshell-settings-xvfb` |
...
| Printers | CUPS availability and bounded summary; fixed `system-config-printer` entry point | Read-only and delegated | CUPS and the trusted Fedora tool own printer discovery, authentication, jobs, queues, and cancellation. |
