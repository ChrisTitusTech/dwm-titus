...
| Input | `dwm-settings-input` over XInput/libinput, `setxkbmap`, and udev hotplug events | Timed per-device preview, reset, XDG persistence, and idempotent session-start apply | Read-only, user-session | Unsupported properties are reported per stable device; disconnects are skipped without affecting other devices or sections | `make check-settings check-quickshell-settings-xvfb`; representative real keyboard/pointer checks |
...
| Hotplug watch | `udevadm monitor --subsystem-match=input` while Input is active | Read-only | Event-driven and section-owned; it stops on section change or Settings close. |
