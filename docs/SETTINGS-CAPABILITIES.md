...
| Appearance and accessibility | Versioned `dwm-settings-appearance` theme snapshot and bounded asset inventory; `dwm-settings-wallpaper`; `Theme.qml`, `dwm-quickshell-controlcenter`, `theme-apply.sh`, `feh`, and delegated `nwg-look` | User theme and wallpaper state; toolkit config; external GTK tool | Read-only, user-session, delegated, unsupported | Invalid theme records, missing integrations, and unavailable wallpaper, font, cursor, icon, GTK, Qt, or compositor selections are attributed per capability; wallpaper state is persisted with bounded preview and recovery, while the Settings presentation, remaining asset mutations, notifications, and accessibility still need contracts | `make check-appearance check-quickshell-controlcenter check-quickshell-qml`; live theme and wallpaper reload |
...
| Wallpaper | `dwm-settings-wallpaper` persisted selection and fit mode, plus the existing randomize action | User-session | Preview rollback is bounded; missing or undecodable saved images fall back to the random-fill session default; missing Feh, directory, or images remain isolated failures. |
| GTK configuration tool | `nwg-look` | Delegated | Optional entry point only. |
| Wallpaper, font, cursor, icon, GTK, Qt, and compositor asset state | `dwm-settings-appearance inventory` version 1.0 plus pane-scoped asset and Picom watchers | Read-only | Candidate output and watch coverage are bounded; missing tools or assets degrade only their capability. Add mutations with explicit preview, reset, and rollback behavior. |
