...
| Desktop font and text size | `dwm-settings-personalization`, GSettings change notifications, and `Theme.qml` | User-session | Appearance exposes one font and text-size section for applications and the shell, including the full 75-200 percent range. Qt native DPI scaling is counted once, and shell controls and panel height accommodate the shared scale; icon glyphs retain their dedicated font. Changes are observed while Settings is closed. Legacy `font.conf` is preserved as a startup fallback when desktop typography cannot be read; the desktop provider takes precedence when available. Existing legacy preview recovery remains accessible. Applications that cache toolkit settings may need reopening. |
...
| Wallpaper, font, cursor, icon, GTK, Qt, and compositor asset state | `dwm-settings-appearance inventory` version 1.0 plus pane-scoped asset and Picom watchers | Read-only | Candidate output and watch coverage are bounded; missing tools or assets degrade only their capability. Appearance shows one font and text-size section, cursor and icon choices, and theme selection. GTK/Qt selectors, keyboard/pointer access summaries, and application status cards are hidden; errors and recovery remain visible. Existing toolkit overrides are preserved. |
