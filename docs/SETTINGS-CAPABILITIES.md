...
| Appearance and accessibility | Versioned `dwm-settings-appearance` theme snapshot and bounded asset inventory; `dwm-settings-wallpaper`; `dwm-settings-font`; `dwm-settings-personalization`; `Theme.qml`, `dwm-quickshell-controlcenter`, `theme-apply.sh`, `feh`, and delegated `nwg-look` | User theme, wallpaper, managed-shell typography, and desktop personalization state; transactional toolkit config; external GTK and Qt tools | Read-only, user-session, delegated, unsupported | Invalid theme records, missing integrations, and unavailable assets are attributed per capability; theme, wallpaper, managed-shell font, and personalization mutations provide bounded recovery, while notifications and accessibility still need contracts | `make check-appearance check-quickshell-controlcenter check-quickshell-qml`; nested-X11 appearance and live toolkit reload |
...
| Font, text scale, cursor, icon, GTK, and Qt choices | `dwm-settings-personalization` validates the bounded inventory choice and delegates fixed apply/reset actions to `dwm-settings-theme`; `theme-apply.sh` consumes versioned persisted overrides; `dwm-xsettings` owns the generated X11 text-scale channel | User-session | Status reports the effective value and persisted override or follow mode separately. The existing theme mutation and integration locks serialize every output. Hash-guarded publish and durable recovery preserve unrelated or concurrent edits. A scoped `xsettingsd` instance exposes verified fixed-point DPI to native GTK applications, refuses to displace an existing XSETTINGS owner, and is stopped on system-follow reset; cursor, GTK, and Qt reset to explicit theme-follow behavior, while font, text scale, and icons reset to system-follow behavior. |
| Advanced GTK and Qt editing | Fixed `delegate gtk\|qt` actions open an installed trusted editor | Delegated | The entry point is unavailable when no supported editor is installed. Advanced editor state remains externally owned and is not interpreted as a Settings transaction. |
