...
| Appearance and accessibility | Versioned `dwm-settings-appearance` theme snapshot and bounded asset inventory; `dwm-settings-wallpaper`; `dwm-settings-font`; `dwm-settings-personalization`; `dwm-panel-settings`; `Theme.qml`, `theme-apply.sh`, `feh`, and delegated `nwg-look` | User theme, wallpaper, managed-shell typography, desktop personalization, and shared panel-widget state; transactional toolkit config; external GTK and Qt tools | Read-only, user-session, delegated, unsupported | Invalid theme records, missing integrations, panel state, and unavailable assets are attributed per capability; appearance and panel mutations provide safe recovery, while notifications and accessibility still need contracts | `make check-appearance check-quickshell-panel-settings check-quickshell-controlcenter check-quickshell-qml`; nested-X11 appearance and live toolkit reload |
