...
| Appearance and accessibility | Versioned `dwm-settings-appearance` theme snapshot and bounded asset inventory; `Theme.qml`, `dwm-quickshell-controlcenter`, `theme-apply.sh`, `feh`, and delegated `nwg-look` | User theme files and toolkit config; session wallpaper; external GTK tool | Read-only, user-session, delegated, unsupported | Invalid theme records, missing integrations, and unavailable wallpaper, font, cursor, icon, GTK, Qt, or compositor selections are attributed per capability; asset mutations, notifications, and accessibility still need settings contracts | `make check-appearance check-quickshell-controlcenter check-quickshell-qml`; live theme reload |
